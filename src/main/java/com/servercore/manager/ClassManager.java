...
public class ClassManager {
    ...
    public enum PlayerClass {
        ...
        CALAMITY_FAMILIAR("灾厄使魔", Material.MAGMA_CREAM, AttributeManager.CoreAttribute.INTELLIGENCE, AttributeManager.CoreAttribute.LUCK,
                List.of("法术伤害通用乘区 = Power * 0.01", "代价: 扣除最大生命值 max(25%, Power * 0.12%)")),
        GAMBLER("赌徒", Material.AMETHYST_SHARD, AttributeManager.CoreAttribute.WILLPOWER, AttributeManager.CoreAttribute.LUCK,
                List.of("额外暴击率 = Power * 0.25%", "额外暴击伤害 = Power * 0.25%", "抢夺等级 = floor(Power / 75)", "代价: 扣除护甲值 max(30%, Power * 0.2%)")),
        ASSASSIN("刺客", Material.NETHERITE_SWORD, AttributeManager.CoreAttribute.TOUGHNESS, AttributeManager.CoreAttribute.LUCK,
                List.of("移动速度 = Power * 0.8", "额外暴击伤害 = Power * 0.35%"));
    ...
    public void openClassSelectionGUI(Player player) {
        ChestGui gui = new ChestGui(3, "✦ 职业大厅 ✦");
        ...
        pane.addItem(new GuiItem(createCurrentClassIcon(player)), 8, 1);
        gui.addPane(pane);
        gui.show(player);
    }
    ...
    public double getBonusBrutality(Player player) {
        return getPlayerClass(player) == PlayerClass.BLOOD_MAGE ? getClassPower(player) * 0.65 : 0.0;
    }

    public double getBonusRegen(Player player) {
        return getPlayerClass(player) == PlayerClass.GUARDIAN ? getClassPower(player) * 0.06 : 0.0;
    }
    ...
    public double getBonusMana(Player player) {
        return getPlayerClass(player) == PlayerClass.PROPHET ? getClassPower(player) * 3.0 : 0.0;
    }

    public double getMagicMultiplierBonus(Player player) {
        return getPlayerClass(player) == PlayerClass.CALAMITY_FAMILIAR ? getClassPower(player) * 0.01 : 0.0;
    }
    ...
    public int getLootLevel(Player player) {
        return getPlayerClass(player) == PlayerClass.GAMBLER ? (int) Math.floor(getClassPower(player) / 75.0) : 0;
    }
