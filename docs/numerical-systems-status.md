...
```text
脱战等待 = max(最小等待, 基础等待 - 意志 × 每点缩短秒数)
脱战回血 = 最大生命 × 配置百分比 + 守护者/附魔等平铺回复
```

- 原版自然回血关闭，饱食度锁定，不再参与回血倍率；
- 玩家造成或受到有效战斗伤害后进入战斗状态，倒计时结束且附近没有敌对单位时开始恢复；
- 意志只缩短脱战等待，不提高回血速度；
- 守护者保留职业回复价值：战斗中可按职业系数获得低速回复，脱战后也可计入平铺回复；
- 详细后勤规则见 `docs/player-logistics-system-status.md`。
...
| 游侠 | 敏捷/意志 | 攻速 `Power×0.2`；移速 `Power×0.6`；命中叠 3 移速，上限 75 |
