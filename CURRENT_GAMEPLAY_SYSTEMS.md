...
`CombatManager` 拦截玩家攻击并负责攻击者侧公式，目标侧统一交给 `DamageService`。它区分：
...
暴击由 ServerCore 自己接管，原版暴击不再直接决定最终伤害。残暴偏向满蓄力平 A 的追击收益。命中武器、附魔和吸血率会在攻击时锁定；有副作用的进攻状态在 `MONITOR` 确认事件成功后才提交。
...
- 近战、弹射物和直接法术闪避
- 盾牌格挡优先于普通减伤
- 自定义护甲减伤公式：`min(95%, armor / (armor + 100))`
- 职业护甲惩罚
- 核心属性带来的魔法减伤
- 生物标签/元素抗性与 DOT 上限
- 承伤附魔、统一被动、灾厄血池与守护者代偿
- 被动复活和凤凰核心

原版护甲减伤已经被剥离。原版防具会通过 `ItemStandardizer` 把面板护甲迁移到 PDC `base_armor`，并写入 0 值属性覆盖，避免 Minecraft 原版护甲和 ServerCore 护甲重复减伤。

所有目标侧结算共用 `DamageService`。护盾、耗魔抵伤、Shade Step、血池以及带冷却的进攻状态先生成计划，只有事件最终未取消时才实际消耗。真实伤害跳过普通防御，但仍受 DOT 安全上限与致死保护约束。
